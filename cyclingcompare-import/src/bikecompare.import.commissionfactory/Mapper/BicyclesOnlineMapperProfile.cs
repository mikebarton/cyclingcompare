...
{
    public class BicyclesOnlineMapperProfile : Profile
    {
        public BicyclesOnlineMapperProfile()
        {
            CreateMap<DataFeedItem, Product>()
                .ForMember(dest => dest.Price, opt => opt.MapFrom(src => src.Price.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()))
                .ForMember(dest => dest.PriceRrp, opt => opt.MapFrom(src => src.PriceRrp.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault()))
                .ForMember(dest => dest.Currency, opt => opt.MapFrom(src => src.Price.Split(' ', StringSplitOptions.RemoveEmptyEntries).LastOrDefault()))
                .ForMember(dest => dest.StockLevel, opt => opt.MapFrom((feedItem, product) =>
                {
                    if (string.Equals(feedItem.StockLevel, "", StringComparison.OrdinalIgnoreCase))
                        return (bool?)null;

                    throw new Exception("non empty string: " + feedItem.StockLevel);                    
                }))
