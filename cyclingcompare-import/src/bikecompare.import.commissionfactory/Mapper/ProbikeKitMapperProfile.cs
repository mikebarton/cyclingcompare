...
{
    public class ProBikeKitMapperProfile : Profile
    {
        public ProBikeKitMapperProfile()
        {
            CreateMap<DataFeedItem, Product>()
                .ForMember(dest => dest.StockLevel, opt => opt.Ignore())
                .ForMember(dest => dest.HasStock, opt => opt.MapFrom((feedItem, product) => !string.Equals(feedItem.StockLevel, "", StringComparison.OrdinalIgnoreCase)))
