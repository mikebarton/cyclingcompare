...
{
    public class WiggleMapperProfile : Profile
    {
        public WiggleMapperProfile()
        {
            ...
            try
            {
                ...
                if (index > 0)
                {
                    ...
                    if (matches.Count > 0)
                    {
                        ...
                        .Select(l => new { Header = l.Match.Value, Value = l.NextMatch == null ? Regex.Match(featuresOn.Substring(l.Match.Index + l.Match.Length), @"[\S]+(?=[A-Z]*)")?.Value : featuresOn.Substring(l.Match.Index + l.Match.Length, l.NextMatch.Index - (l.Match.Index + l.Match.Length)) }).ToSafeDictionary(x => x.Header, x => x.Value);
                        return specDict;
                    }
                }
                return null;
