...
export default function ProductItem({item}){
    ...
    function RenderDiscount(){
        if((item.minPriceRrp) && (item.minPriceRrp > item.minPrice) && (item.minPriceRrp > 0) && (item.minPrice !== item.minPriceRrp)){
            return (
    ...
    return (
        <div key={item.productId} className={styles.productSummaryContainer}>
            ...
            <div className={styles.textContainer}>
                ...
                <div className={styles.priceLabel}>from <NumberFormat value={item.minPrice} displayType={'text'} thousandSeparator={true} prefix={item.currencyCode === 'GBP'? '£' : '$'}/></div>
            </div>
            <RenderButton/>
            <RenderDiscount/>
        </div>
